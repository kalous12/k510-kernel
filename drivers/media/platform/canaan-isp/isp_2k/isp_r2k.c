...
int isp_r2k_wrap_reset(struct k510_isp_device *isp)
{
	...
	isp_reg_writel(isp,stData,ISP_IOMEM_R2K_WRAP,ISP_WRAP_DMA_SWRST_CTL);
	//
	isp_r2k_wrap_SetConfigDone(isp,0);
	return 0;
}
//
static int isp_r2k_wrap_config(struct isp_r2k_device *r2k,struct isp_wrap_cfg_info *isp_wrap_cfg)
{
...
void k510isp_r2k_irq_enable(struct k510_isp_device *isp,struct isp_irq_info *irq_info)
{
	...
	static struct isp_wrap_intwr1_info intWr1En;
	...
	dev_dbg(isp->dev,"%s:intWr1En(0x%x)\n",__func__,intWr1En);
...
static int isp_r2k_core_GetAeSts(struct k510_isp_device *isp,struct k510isp_ae_stats *ae_stats)
{
	...
	Isp_Drv_R2k_Core_GetAeSts(isp,&pAeSts);
...
//
/*
 * isp_r2k_set_fbc_outaddr - Set memory address to save output image
 * @r2k: Pointer to ISP r2k device.
 * @addr: ISP MMU Mapped 32-bit memory address aligned on 32 byte boundary.
 *
 * Sets the memory address where the output will be saved.
 */
static void isp_r2k_set_fbc_outaddr(struct isp_r2k_device *r2k, u32 addr)
{
	struct k510_isp_device *isp = to_isp_device(r2k);

	isp_reg_writel(isp,addr,ISP_IOMEM_R2K_WRAP,ISP_WRAP_MAIN_Y_BUF0_BASE);
	isp_reg_writel(isp,addr,ISP_IOMEM_R2K_WRAP,ISP_WRAP_MAIN_Y_BUF1_BASE);
...
/*
 * isp_r2k_set_main_outaddr - Set memory address to save output image
 * @r2k: Pointer to ISP r2k device.
 * @addr: ISP MMU Mapped 32-bit memory address aligned on 32 byte boundary.
 *
 * Sets the memory address where the output will be saved.
 */
static void isp_r2k_set_main_outaddr(struct isp_r2k_device *r2k, u32 addr)
{
	struct k510_isp_device *isp = to_isp_device(r2k);
...
/*
 * isp_r2k_set_out0_outaddr - Set memory address to save output image
 * @r2k: Pointer to ISP r2k device.
 * @addr: ISP MMU Mapped 32-bit memory address aligned on 32 byte boundary.
 *
 * Sets the memory address where the output will be saved.
 */
static void isp_r2k_set_out0_outaddr(struct isp_r2k_device *r2k, u32 addr)
{
	struct k510_isp_device *isp = to_isp_device(r2k);
	struct isp_wrap_cfg_info *isp_wrap_cfg = &r2k->isp_cfg.isp_wrap_cfg;
 ...
 * Sets the memory address where the output will be saved.
 */
static void isp_r2k_set_out1_outaddr(struct isp_r2k_device *r2k, u32 addr)
{
	struct k510_isp_device *isp = to_isp_device(r2k);
	struct isp_wrap_cfg_info *isp_wrap_cfg = &r2k->isp_cfg.isp_wrap_cfg;
	struct isp_wrap_ds1_info *ds1_info = &isp_wrap_cfg->ds1Info;
	isp_reg_writel(isp,addr,ISP_IOMEM_R2K_WRAP,ISP_WRAP_DS1_Y_BUF0_BASE);
	isp_reg_writel(isp,addr,ISP_IOMEM_R2K_WRAP,ISP_WRAP_DS1_Y_BUF1_BASE);
	//dev_dbg(r2k->isp->dev,"%s:line_stride (%d),Width(%d),Height(%d)\n",\
	__func__,ds1_info->ds1_line_stride,ds1_info->ds1_size.Width,ds1_info->ds1_size.Height);
...
/*
 * isp_r2k_set_out2_outaddr - Set memory address to save output image
 * @r2k: Pointer to ISP r2k device.
 * @addr: ISP MMU Mapped 32-bit memory address aligned on 32 byte boundary.
 *
 * Sets the memory address where the output will be saved.
 */
static void isp_r2k_set_out2_outaddr(struct isp_r2k_device *r2k, u32 addr)
{
	struct k510_isp_device *isp = to_isp_device(r2k);
	struct isp_wrap_cfg_info *isp_wrap_cfg = &r2k->isp_cfg.isp_wrap_cfg;
...
/****************************************************************************/
static void isp_r2k_set_inaddr(struct isp_r2k_device *r2k, u32 addr)
{
	struct k510_isp_device *isp = to_isp_device(r2k);
...
static void isp_r2k_enable(struct isp_r2k_device *r2k)
{
	struct k510_isp_device *isp = to_isp_device(r2k);
}
/*
 * ISP_R2K_WRAP_print_status - Prints the values of the r2k wrap module registers.
 */
...
void isp_r2k_wrap_print_status(struct isp_r2k_device *r2k)
{
	...
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_SWRST_CTL     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_SWRST_CTL     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_CTL_MODE     			);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_PIPE_CLK_CTL     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_EN_CTL     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_3DNR_WDR_FORMAT_CTL   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_PIXEL_FORMAT_CTL     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_MAIN_OUT_SIZE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS0_OUT_SIZE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS1_OUT_SIZE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_OUT_SIZE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_BUF_SIZE_MASK0    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_BUF_SIZE_MASK1    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_BUF_SIZE_MASK2    	);

	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_ARB_MODE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_WEIGHT_0     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_WEIGHT_1     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_WEIGHT_0     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_WEIGHT_1     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_PRIORITY_0     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_PRIORITY_0     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_DMA_WR_CH_ID_0     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_DMA_RD_CH_ID_0     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_WEIGHT_2     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_WEIGHT_3     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_WEIGHT_2     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_WEIGHT_3     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_PRIORITY_1     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_PRIORITY_1     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_DMA_WR_CH_ID_1     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_DMA_RD_CH_ID_1     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_WDR_LONG_BUF_BASE     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_WDR_LONG_SIZE_CFG     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_3DNR_Y_BUF_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_3DNR_UV_BUF_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_3DNR_STRIDE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_LDC_Y_BUF_BASE    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_LDC_UV_BUF_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_LDC_STRIDE	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_MAIN_Y_BUF0_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_MAIN_Y_BUF1_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_MAIN_UV_BUF0_BASE     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_MAIN_UV_BUF1_BASE     );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_MAIN_STRIDE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS0_Y_BUF0_BASE    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS0_Y_BUF1_BASE    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS0_UV_BUF0_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS0_UV_BUF1_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS0_STRIDE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS1_Y_BUF0_BASE    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS1_Y_BUF1_BASE    	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS1_UV_BUF0_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS1_UV_BUF1_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS1_STRIDE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_R_BUF0_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_R_BUF1_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_G_BUF0_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_G_BUF1_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_B_BUF0_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_B_BUF1_BASE     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_STRIDE0    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DS2_STRIDE1    		);

	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH0_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH1_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH2_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH3_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH4_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH5_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH6_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH7_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH8_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH9_CFG     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH10_CFG    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH11_CFG    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH12_CFG    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH13_CFG    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH14_CFG    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH15_CFG    		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH0_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH1_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH2_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH3_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH4_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH5_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH6_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH7_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH8_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH9_ERR_DEC_CFG   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH10_ERR_DEC_CFG  );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH11_ERR_DEC_CFG  );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH12_ERR_DEC_CFG  );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH13_ERR_DEC_CFG  );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH14_ERR_DEC_CFG  );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH15_ERR_DEC_CFG  );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH0_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH1_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH2_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH3_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH4_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH5_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH6_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH7_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH8_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH9_ERR_STATUS    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH10_ERR_STATUS   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH11_ERR_STATUS   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH12_ERR_STATUS   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH13_ERR_STATUS   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH14_ERR_STATUS   );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_CH15_ERR_STATUS   );

	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_CONFIG_DONE     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_CORE_INT_CTL     		);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_INT_STATUS0    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_INT_MASK0     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_INT_STATUS1    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_WR_INT_MASK1     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_INT_STATUS0    );
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_DMA_RD_INT_MASK0     	);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_WRAP_AXI_CTL     			);

	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_VERSION     				);
	ISP_R2K_WRAP_PRINT_REGISTER(isp,ISP_RELEASE_INFO     			);
...
static void isp_r2k_core_print_status(struct isp_r2k_device *r2k)
{
	...
	dev_dbg(isp->dev, "-------------ISP R2K CORE Register dump start----------\n");
	//itc
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_ITC_CTL               		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_IMAGE_HEIGHT          		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_IMAGE_WIDTH           		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_IMAGE_V_START         		);
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_LSC_R_RATIO           		);
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_WIN_H_END          		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_WIN_V_END          		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_TAR_BR             		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_TAR_BR_RANGE       		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_L_EX_TIME          		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_M_EX_TIME          		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_S_EX_TIME          		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_AGC                		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_ADJUST_CTL         		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_ADJUST_STEP_MAX    		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_EX_VALUE_MAX       		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_EX_VALUE_MID       		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_EX_VALUE_MIN       		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_GAIN_MAX           		);
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_GAIN_MID           		);
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_APE_COEFF_SPEED            );
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_SHORT_CUR_EX            );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_CUR_DIGITAL_GAIN        );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_CUR_AVE_BRIGHTNESS      );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AE_CUR_DN_STATUS           );
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_RR_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_RG_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_RB_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_GR_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_GG_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_GB_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_BR_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_BG_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_BB_COFF                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CCM_CORRECT_COFF           );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_WIN_H_START            );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_WIN_V_START            );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_WIN_H_END              );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_WIN_V_END              );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_CORRECT_DIFF_TH        );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RES_TIME               );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_HIST_TH                );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_GAIN_ADJUST        );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_GREEN_GAIN_ADJUST      );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_GAIN_ADJUST       );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_MAX_VALUE          );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_MAX_VALUE         );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_MIN_VALUE          );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_MIN_VALUE         );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_OBJ_VALUE          );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_OBJ_VALUE         );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_HIST_VALUE        );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_HIST_PIXEL        );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_HIST_VALUE         );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_HIST_PIXEL         );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BYPASS_BLUE_HIST_VALUE );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BYPASS_BLUE_HIST_PIXEL );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BYPASS_RED_HIST_VALUE  );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BYPASS_RED_HIST_PIXEL  );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_RED_VALUE              );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_GREEN_VALUE            );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AWB_BLUE_VALUE             );
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CSC_R2Y_10                 );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CSC_R2Y_11                 );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_CSC_R2Y_12                 );
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_ADA_WIN_H_START            );
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_ENH_NR_TH                  );
	...
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AF_STAT_WIN_H_END          );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AF_STAT_WIN_V_END          );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AF_MID_FRQ_DATA            );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AF_HIGH_FRQ_DATA           );
	ISP_R2K_CORE_PRINT_REGISTER(isp,ISP_CORE_AF_STAT_PIXEL_NUM          );
...
static void isp_r2k_ds_print_status(struct isp_r2k_device *r2k)
{
	...
	dev_dbg(isp->dev, "-------------isp r2k ds Register dump start----------\n");
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_INPUT_SIZE);
	//RGB2YUV
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF00);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF01);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF02);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF03);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF10);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF11);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF12);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF13);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF20);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF21);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF22);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_RGB2YUV_COEFF23);
	//YUV2RGB
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF00);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF01);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF02);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF03);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF10);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF11);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF12);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF13);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF20);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF21);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF22);
	ISP_R2K_DS_PRINT_REGISTER(isp,ISP_DS_OSD_YUV2RGB_COEFF23);
	//DS0
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_VSCALE_PARM);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_HSCALE_PARM);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_SIZE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_FORMAT     );
	//DS0 OSD0
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD0_Y_RANGE     	);
	//DS0 OSD1
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_X_RANGE    	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD1_Y_RANGE    	);
	//DS0 OSD2
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS0_OSD2_Y_RANGE     	);
	//DS1
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_VSCALE_PARM);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_HSCALE_PARM);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_SIZE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_FORMAT     );
	//DS1 0SD0
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD0_Y_RANGE     	);
	//DS1 0SD1
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD1_Y_RANGE     	);
	//DS1 0SD2
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS1_OSD2_Y_RANGE     	);
	//DS2
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_VSCALE_PARM);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_HSCALE_PARM);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_SIZE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_FORMAT     );
	//DS2 OSD0
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_INFO    		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_SIZE    		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD0_Y_RANGE     	);
	//DS2 OSD1
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_INFO    		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_SIZE    		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD1_Y_RANGE     	);
	//DS2 OSD2
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_INFO     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_SIZE     		);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_BUF_ADDR0     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_ALPHA_BUF_ADDR0);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_BUF_ADDR1     );
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_ALPHA_BUF_ADDR1);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_DMA_CTL     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_STRIDE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_X_RANGE     	);
	ISP_R2K_DS_PRINT_REGISTER(isp, ISP_DS2_OSD2_Y_RANGE     	);
...
/*
 * isp_r2k_remap_print_status - Prints the values of the r2k remap module registers.
 */
...
static void isp_r2k_remap_print_status(struct isp_r2k_device *r2k)
{
	...
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_01_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_01_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_02_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_02_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_03_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_03_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_04_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_04_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_05_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_05_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_06_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_06_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_07_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_07_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_08_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_08_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_09_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_09_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_10_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_10_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_11_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_11_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_12_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_12_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_13_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_13_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_14_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_14_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_15_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_15_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_16_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_16_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_17_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_17_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_18_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_18_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_19_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_19_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_20_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_20_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_21_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_21_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_22_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_22_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_23_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_23_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_24_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_24_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_25_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_25_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_26_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_26_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_27_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_27_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_28_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_28_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_29_0_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_29_1_CTRL);
	ISP_R2K_MAIN_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_30_0_CTRL);
	...
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_02_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_02_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_03_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_03_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_04_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_04_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_05_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_05_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_06_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_06_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_07_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_07_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_08_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_08_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_09_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_09_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_10_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_10_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_11_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_11_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_12_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_12_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_13_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_13_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_14_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_14_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_15_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_15_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_16_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_16_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_17_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_17_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_18_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_18_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_19_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_19_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_20_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_20_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_21_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_21_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_22_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_22_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_23_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_23_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_24_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_24_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_25_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_25_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_26_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_26_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_27_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_27_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_28_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_28_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_29_0_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_29_1_CTRL);
	ISP_R2K_OUT0_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_30_0_CTRL);
	...
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_01_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_01_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_02_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_02_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_03_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_03_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_04_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_04_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_05_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_05_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_06_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_06_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_07_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_07_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_08_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_08_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_09_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_09_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_10_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_10_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_11_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_11_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_12_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_12_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_13_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_13_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_14_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_14_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_15_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_15_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_16_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_16_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_17_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_17_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_18_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_18_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_19_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_19_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_20_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_20_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_21_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_21_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_22_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_22_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_23_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_23_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_24_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_24_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_25_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_25_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_26_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_26_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_27_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_27_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_28_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_28_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_29_0_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_29_1_CTRL);
	ISP_R2K_OUT1_REMAP_PRINT_REGISTER(isp,ISP_FILL_INFO_AREA_30_0_CTRL);
...
static void r2k_isr_main_buffer(struct isp_r2k_device *r2k)
{
	...
	if (list_empty(&r2k->video_out[MAIN_VIDEO].dmaqueue))
		return 0;

	if (r2k->state == ISP_PIPELINE_STREAM_CONTINUOUS && r2k->underrun) {
		r2k->underrun = 0;
		return 1;
	}
...
static void r2k_isr_out0_buffer(struct isp_r2k_device *r2k)
{
	...
	if (list_empty(&r2k->video_out[DS0_VIDEO].dmaqueue))
		return 0;
...
static void r2k_isr_out1_buffer(struct isp_r2k_device *r2k)
{
	...
	if (list_empty(&r2k->video_out[DS1_VIDEO].dmaqueue))
		return 0;
...
static void r2k_isr_out2_buffer(struct isp_r2k_device *r2k)
{
	struct k510isp_pipeline *pipe = to_isp_pipeline(&r2k->subdev.entity);
	struct k510_isp_device *isp = to_isp_device(r2k);
	struct k510isp_buffer *buffer;

	if (list_empty(&r2k->video_out[DS2_VIDEO].dmaqueue))
		return 0;
 ...
 */
static int r2k_video_queue(struct k510isp_video *video, struct k510isp_buffer *buffer)
{
	 ...
	 * sync mode as no r2k interrupt is generated when the r2k is stopped
	 * in that case.
	 */
	spin_lock_irqsave(&r2k->lock, flags);
	if (r2k->state == ISP_PIPELINE_STREAM_CONTINUOUS && !r2k->running )
		restart = true;
	else
		r2k->underrun = 1;
	spin_unlock_irqrestore(&r2k->lock, flags);
...
static const struct k510isp_video_operations r2k_video_ops = {
	.queue = r2k_video_queue,
};
...
/*
 * r2k_ioctl - r2k module private ioctl's
 * @sd: ISP r2k V4L2 subdevice
 * @cmd: ioctl command
 * @arg: ioctl argument
 *
 * Return 0 on success or a negative error code otherwise.
 */
static long r2k_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct isp_r2k_device *r2k = v4l2_get_subdevdata(sd);
	int ret;
	dev_dbg(r2k->isp->dev,"%s:cmd(0x%x)\n",__func__,cmd);
	struct isp_wrap_cfg_info *isp_wrap_cfg = &r2k->isp_cfg.isp_wrap_cfg;
	struct isp_core_cfg_info *isp_core_cfg = &r2k->isp_cfg.isp_core_cfg;
		...
		struct v4l2_event_subscription *sub)
{
	...
	return v4l2_event_subscribe(fh, sub, ISP_R2K_NEVENTS, NULL);
}
...
/* --------------------------------------------------------------------------
 * r2k video device node
 *--------------------------------------------------------------------------/

/*
 * r2k_s_stream - Enable/Disable streaming on r2k subdev
...
static const unsigned int r2k_fmts[] = {
...
{
	if (which == V4L2_SUBDEV_FORMAT_TRY)
		return v4l2_subdev_get_try_crop(&r2k->subdev, cfg, ISP_R2K_PAD_MAIN_SOURCE);
	else
		return &r2k->crop_in;
}

/*
 * r2k_try_format - Try video format on a pad
 * @r2k: ISP r2k device
...
/*
 * r2k_try_crop - Validate a crop rectangle
 * @r2k: ISP r2k device
 * @sink: format on the sink pad
 * @crop: crop rectangle to be validated
 */
static void r2k_try_crop(struct isp_r2k_device *r2k,const struct v4l2_mbus_framefmt *sink,struct v4l2_rect *crop)
{
	...
	crop->left = clamp_t(u32, crop->left, 0, sink->width - R2K_MIN_WIDTH);
	crop->top = clamp_t(u32, crop->top, 0, sink->height - R2K_MIN_HEIGHT);
...
/*
 * r2k_enum_mbus_code - Handle pixel format enumeration
 * @sd     : pointer to v4l2 subdev structure
 * @cfg : V4L2 subdev pad configuration
		...
		struct v4l2_subdev_frame_size_enum *fse)
{
	...
	format.width = 1;
	format.height = 1;
	r2k_try_format(r2k, cfg, fse->pad, &format, fse->which);
...
/*
 * r2k_get_selection - Retrieve a selection rectangle on a pad
 * @sd: ISP r2k V4L2 subdevice
 * @cfg: V4L2 subdev pad configuration
...
/*
 * r2k_set_selection - Set a selection rectangle on a pad
 * @sd: ISP r2k V4L2 subdevice
 * @cfg: V4L2 subdev pad configuration
 * @sel: Selection rectangle
 *
		...
		struct v4l2_subdev_selection *sel)
{
	...
	if (sel->flags & V4L2_SEL_FLAG_KEEP_CONFIG) {
		sel->r = *__r2k_get_crop(r2k, cfg, sel->which);
		dev_dbg(r2k->isp->dev,"%s:V4L2_SEL_FLAG_KEEP_CONFIG\n",__func__);
		return 0;
	}

	format = __r2k_get_format(r2k, cfg, ISP_R2K_PAD_SINK, sel->which);
	r2k_try_crop(r2k, format, &sel->r);
	*__r2k_get_crop(r2k, cfg, sel->which) = sel->r;
	...
	if( sel->pad == ISP_R2K_PAD_DS0_SOURCE)
	{
		format = __r2k_get_format(r2k, cfg, sel->pad, sel->which);
		r2k_try_format(r2k, cfg, sel->pad, format, sel->which);
	}
...
/*
 * r2k_get_format - Retrieve the video format on a pad
 * @sd : ISP r2k V4L2 subdevice
 * @cfg: V4L2 subdev pad configuration
 * @fmt: Format
 *
		...
		struct v4l2_subdev_format *fmt)
{
	...
	dev_dbg(r2k->isp->dev,"%s:start\n",__func__);
	format = __r2k_get_format(r2k, cfg, fmt->pad, fmt->which);
	if (format == NULL)
	{
		dev_err(r2k->isp->dev,"%s:format == NULL\n",__func__);
		return -EINVAL;
	}
...
/*
 * r2k_set_format - Set the video format on a pad
 * @sd : ISP r2k V4L2 subdevice
 * @cfg: V4L2 subdev pad configuration
 * @fmt: Format
 *
		...
		struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad >ISP_R2K_PADS_NUM)
	{
		dev_warn(r2k->isp->dev, "%s:pad error!\n",__func__);
		return -EINVAL;
	}
	if (fmt->pad == ISP_R2K_PAD_SINK) {
		...
		r2k->formats[ISP_R2K_PAD_SINK].field = V4L2_FIELD_NONE;
		r2k->formats[ISP_R2K_PAD_SINK].colorspace =V4L2_COLORSPACE_SRGB;
		r2k->formats[ISP_R2K_PAD_SINK].code = format->code;
	}

	if (fmt->pad == ISP_R2K_PAD_MAIN_SOURCE) {
		/* Reset the crop rectangle. */
		crop = &r2k->crop_out;
		crop->left = 0;
		crop->top = 0;
		...
		format = &fmt->format;
		r2k->formats[ISP_R2K_PAD_MAIN_SOURCE].width = format->width;
	...
	if (fmt->pad == ISP_R2K_PAD_DS0_SOURCE) {
		...
		format = &fmt->format;
		r2k->formats[ISP_R2K_PAD_DS0_SOURCE].width = format->width;
		r2k->formats[ISP_R2K_PAD_DS0_SOURCE].height = format->height;
		r2k->formats[ISP_R2K_PAD_DS0_SOURCE].field = V4L2_FIELD_NONE;
		r2k->formats[ISP_R2K_PAD_DS0_SOURCE].colorspace =V4L2_COLORSPACE_SRGB;
		r2k->formats[ISP_R2K_PAD_DS0_SOURCE].code = format->code;
	}

	if (fmt->pad == ISP_R2K_PAD_DS1_SOURCE) {
		format = &fmt->format;
		r2k->formats[ISP_R2K_PAD_DS1_SOURCE].width = format->width;
		r2k->formats[ISP_R2K_PAD_DS1_SOURCE].height = format->height;
		r2k->formats[ISP_R2K_PAD_DS1_SOURCE].field = V4L2_FIELD_NONE;
		r2k->formats[ISP_R2K_PAD_DS1_SOURCE].colorspace =V4L2_COLORSPACE_SRGB;
		r2k->formats[ISP_R2K_PAD_DS1_SOURCE].code = format->code;
	}

	if (fmt->pad == ISP_R2K_PAD_DS2_SOURCE) {

		format = &fmt->format;
		r2k->formats[ISP_R2K_PAD_DS2_SOURCE].width = format->width;
		r2k->formats[ISP_R2K_PAD_DS2_SOURCE].height = format->height;
		r2k->formats[ISP_R2K_PAD_DS2_SOURCE].field = V4L2_FIELD_NONE;
		r2k->formats[ISP_R2K_PAD_DS2_SOURCE].colorspace =V4L2_COLORSPACE_SRGB;
		r2k->formats[ISP_R2K_PAD_DS2_SOURCE].code = format->code;
	}
...
/*
 * r2k_init_formats - Initialize formats on all pads
 * @sd: ISP r2k V4L2 subdevice
 ...
 */
static int r2k_init_formats(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct isp_r2k_device *r2k = v4l2_get_subdevdata(sd);
	struct v4l2_subdev_format format;
	int pad;

	for(pad = ISP_R2K_PAD_SINK;pad<ISP_R2K_PAD_DS2_SOURCE+1;pad++)
	{
...
static const struct v4l2_subdev_core_ops r2k_v4l2_core_ops = {
	.ioctl = r2k_ioctl,
	.subscribe_event = r2k_subscribe_event,
	.unsubscribe_event = r2k_unsubscribe_event,
};
/* subdev video operations */
static const struct v4l2_subdev_video_ops r2k_v4l2_video_ops = {
	.s_stream = r2k_set_stream,
};

/* subdev pad operations */
static const struct v4l2_subdev_pad_ops r2k_v4l2_pad_ops = {
	.enum_mbus_code = r2k_enum_mbus_code,
	.enum_frame_size = r2k_enum_frame_size,
	.get_fmt = r2k_get_format,
	.set_fmt = r2k_set_format,
	.get_selection = r2k_get_selection,
	.set_selection = r2k_set_selection,
	.link_validate = r2k_link_validate,
};

/* subdev operations */
static const struct v4l2_subdev_ops r2k_v4l2_ops = {
	.core = &r2k_v4l2_core_ops,
	.video = &r2k_v4l2_video_ops,
	.pad = &r2k_v4l2_pad_ops,
};

/* subdev internal operations */
static const struct v4l2_subdev_internal_ops r2k_v4l2_internal_ops = {
	.open = r2k_init_formats,
};
/* -----------------------------------------------------------------------------
 * Media entity operations
 *-----------------------------------------------------------------------------/
/*
 * r2k_link_setup - Setup r2k connections.
		...
		const struct media_pad *remote, u32 flags)
{
	...
	dev_dbg(r2k->isp->dev,"%s:index (%d) start\n",__func__,index);
...
/* media operations */
static const struct media_entity_operations r2k_media_ops = {
	.link_setup = r2k_link_setup,
	.link_validate = v4l2_subdev_link_validate,
};

/*
 * k510isp_r2k_unregister_entities - Unregister media entities: subdev
 * @ccp2: Pointer to ISP r2k device
 */
void k510isp_r2k_unregister_entities(struct isp_r2k_device *r2k)
{
	dev_dbg(r2k->isp->dev,"%s:start\n",__func__);
	v4l2_device_unregister_subdev(&r2k->subdev);
	...
	k510isp_video_unregister(&r2k->video_out[DS2_VIDEO]);
}
/*
 * r2k_register_entities - Register the subdev media entity
 * @r2k: Pointer to ISP r2k device
		...
		struct v4l2_device *vdev)
{
	...
	ret = v4l2_device_register_subdev(vdev, &r2k->subdev);
	if (ret < 0)
	{
...
error:
	...
	dev_dbg(r2k->isp->dev,"%s: end ret:(%d)\n",__func__,ret);
	return ret;
}
/* -----------------------------------------------------------------------------
 * ISP r2k initialisation and cleanup
 *-----------------------------------------------------------------------------/
/*
 * r2k_init_entities - Initialize r2k subdev and media entity.
 * @r2k: Pointer to ISP r2k device
 * return negative error code or zero on success
 */
static int k510isp_r2k_init_entities(struct isp_r2k_device *r2k)
{
	struct v4l2_subdev *sd = &r2k->subdev;
	struct media_pad *pads = r2k->pads;
	...
	v4l2_subdev_init(sd, &r2k_v4l2_ops);
	sd->internal_ops = &r2k_v4l2_internal_ops;
	strlcpy(sd->name, "K510 ISP R2K", sizeof(sd->name));
	sd->grp_id = 1 << 16;   /* group ID for isp subdevs */
	v4l2_set_subdevdata(sd, r2k);

	sd->flags |= V4L2_SUBDEV_FL_HAS_EVENTS|V4L2_SUBDEV_FL_HAS_DEVNODE;

	pads[ISP_R2K_PAD_SINK].flags = MEDIA_PAD_FL_SINK| MEDIA_PAD_FL_MUST_CONNECT;
	pads[ISP_R2K_PAD_MAIN_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	pads[ISP_R2K_PAD_DS0_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	pads[ISP_R2K_PAD_DS1_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	pads[ISP_R2K_PAD_DS2_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	...
	me->ops = &r2k_media_ops;
	dev_dbg(isp->dev,"%s:media_entity_pads_init start\n",__func__);
	ret = media_entity_pads_init(me,ISP_R2K_PADS_NUM, pads);
	if (ret < 0)
	{
	...
	//MAIN
	r2k->video_out[MAIN_VIDEO].type = V4L2_BUF_TYPE_VIDEO_CAPTURE;//V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE;//V4L2_BUF_TYPE_VIDEO_OUTPUT;
	r2k->video_out[MAIN_VIDEO].bpl_alignment = 32;//16;//32;
	r2k->video_out[MAIN_VIDEO].bpl_max = 0xffffffe0;//0xfffffff0; //0xffffffe0
	r2k->video_out[MAIN_VIDEO].isp = to_isp_device(r2k);
	r2k->video_out[MAIN_VIDEO].ops = &r2k_video_ops;
	r2k->video_out[MAIN_VIDEO].capture_mem = PAGE_ALIGN(4096 * 4096) * 3;
	//r2k->output |= ISP_R2K_OUTPUT_MAIN_MEM;

	ret = k510isp_video_init(&r2k->video_out[MAIN_VIDEO], "R2K");
	if (ret < 0)
	{
	...
	r2k->video_out[DS0_VIDEO].bpl_max = 0xffffffe0;//0xfffffff0; //0xffffffe0
	r2k->video_out[DS0_VIDEO].isp = to_isp_device(r2k);
	r2k->video_out[DS0_VIDEO].ops = &r2k_video_ops;
	r2k->video_out[DS0_VIDEO].capture_mem = PAGE_ALIGN(4096 * 4096) * 3;
	//r2k->output |= ISP_R2K_OUTPUT_DS0_MEM;

	ret = k510isp_video_init(&r2k->video_out[DS0_VIDEO], "R2K_DS0");
	if (ret < 0)
	{
		dev_err(isp->dev,"%s:k510isp_video_init ret:%d\n",__func__,ret);
	...
	r2k->video_out[DS1_VIDEO].bpl_max = 0xffffffe0;//0xfffffff0; //0xffffffe0
	r2k->video_out[DS1_VIDEO].isp = to_isp_device(r2k);
	r2k->video_out[DS1_VIDEO].ops = &r2k_video_ops;
	r2k->video_out[DS1_VIDEO].capture_mem = PAGE_ALIGN(4096 * 4096) * 3;
	//r2k->output |= ISP_R2K_OUTPUT_DS1_MEM;

	ret = k510isp_video_init(&r2k->video_out[DS1_VIDEO], "R2K_DS1");
	if (ret < 0)
	{
		dev_err(isp->dev,"%s:k510isp_video_init ret:%d\n",__func__,ret);
	...
	r2k->video_out[DS2_VIDEO].bpl_max = 0xffffffe0;//0xfffffff0; //0xffffffe0
	r2k->video_out[DS2_VIDEO].isp = to_isp_device(r2k);
	r2k->video_out[DS2_VIDEO].ops = &r2k_video_ops;
	r2k->video_out[DS2_VIDEO].capture_mem = PAGE_ALIGN(4096 * 4096) * 3;
	//r2k->output |= ISP_R2K_OUTPUT_DS2_MEM;
	dev_dbg(isp->dev,"%s:r2k->output(0x%x)\n",__func__,r2k->output);
	ret = k510isp_video_init(&r2k->video_out[DS2_VIDEO], "R2K_DS2");
	if (ret < 0)
	{
		dev_err(isp->dev,"%s:k510isp_video_init ret:%d\n",__func__,ret);
...
error_ds1_video:
	k510isp_video_cleanup(&r2k->video_out[DS1_VIDEO]);
error_ds0_video:
	k510isp_video_cleanup(&r2k->video_out[DS0_VIDEO]);
error_main_video:
	k510isp_video_cleanup(&r2k->video_out[MAIN_VIDEO]);
error_video:
	media_entity_cleanup(&r2k->subdev.entity);
	dev_dbg(isp->dev,"%s: start ret:(%d)\n",__func__,ret);
	return ret;
}
/*
 * r2k_init - r2k initialization.
...
int k510isp_r2k_init(struct k510_isp_device *isp)
{
	...
	r2k->isp = isp;
	spin_lock_init(&r2k->lock);
	init_waitqueue_head(&r2k->wait);
	mutex_init(&r2k->ioctl_lock);
	r2k->state = ISP_PIPELINE_STREAM_STOPPED;

	unsigned int pad;
	for(pad = ISP_F2K_PAD_SINK;pad < ISP_F2K_PAD_DS2_SOURCE+1;pad++)
	{
		r2k->formats[pad].code = MEDIA_BUS_FMT_SRGGB10_1X10;
		r2k->formats[pad].width = 1920;
		r2k->formats[pad].height = 1080;
	}

	ret = k510isp_r2k_init_entities(r2k);
	if (ret < 0){
...
/*
 * k510isp_r2k_cleanup - isp r2k module cleanup.
