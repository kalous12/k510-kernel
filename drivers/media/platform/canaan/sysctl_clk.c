...
volatile sysctl_clk_t* sysctl_clk ;//= (volatile sysctl_clk_t*)(0x97000000+0x1000); //get_base_addr() ;//(volatile sysctl_clk_t*)SYSCTL_CLK_BASE_ADDR;0x97000000+0x1000, 0x1000 
EXPORT_SYMBOL_GPL(sysctl_clk);
...
EXPORT_SYMBOL_GPL(sysctl_boot_set_root_clk_bypass);
/* 
...
EXPORT_SYMBOL_GPL(sysctl_boot_get_root_clk_en);
/* 
   Enable pll, enable 25M clock&pll, only when both 25m clock and PLL are enabled, can PLL output clock
*/
...
EXPORT_SYMBOL_GPL(sysctl_boot_set_root_clk_en);
/* PLL power supply */
/*
...
EXPORT_SYMBOL_GPL(sysctl_boot_set_root_clk_pwroff);
...
EXPORT_SYMBOL_GPL(sysctl_boot_get_root_clk_lock);
/* get PLL output frequency */
uint32_t sysctl_boot_get_root_clk_freq(sysctl_clk_node_e clk)
{

}
EXPORT_SYMBOL_GPL(sysctl_boot_get_root_clk_freq);
...
EXPORT_SYMBOL_GPL(sysctl_boot_set_root_clk_freq);
...
EXPORT_SYMBOL_GPL(sysctl_clk_set_leaf_parent);
...
EXPORT_SYMBOL_GPL(sysctl_clk_get_leaf_parent);
...
EXPORT_SYMBOL_GPL(sysctl_clk_set_leaf_en);
...
EXPORT_SYMBOL_GPL(sysctl_clk_get_leaf_en);
...
EXPORT_SYMBOL_GPL(sysctl_clk_set_leaf_div);


uint32_t sysctl_clk_get_leaf_div(sysctl_clk_node_e leaf)
{
    ...
    switch(leaf)
    {
        /*---------------------------AX25MP------------------------------------*/
        case SYSCTL_CLK_AX25M_SRC:
            return (120 - ((sysctl_clk->ax25m_clk_cfg >> 4) & 0xF)) / 120;
        case SYSCTL_CLK_AX25M_CORE_0:
        case SYSCTL_CLK_AX25M_CORE_1:
        case SYSCTL_CLK_AX25M_CORE_0_DC:
        case SYSCTL_CLK_AX25M_CORE_1_DC:
        case SYSCTL_CLK_AX25M_MCTL:
            return 1;
        case SYSCTL_CLK_AX25M_MTIMER:
            return 10/(((sysctl_clk->ax25m_mtimer_clk_cfg >> 2) & 0x1F) + 1);

        /*---------------------------AX25P------------------------------------*/
        case SYSCTL_CLK_AX25P_SRC:
            return (60 - ((sysctl_clk->ax25p_clk_cfg >> 4) & 0x7)) / 60;
        case SYSCTL_CLK_AX25P_CORE:
        case SYSCTL_CLK_AX25P_LM:
            return 1;
        case SYSCTL_CLK_AX25P_MTIMER:
            return 10/(((sysctl_clk->ax25p_mtimer_clk_cfg >> 2) & 0x1F) + 1);

        /*---------------------------GNNE------------------------------------*/
        case SYSCTL_CLK_GNNE_SYS:
            return (60 - ((sysctl_clk->gnne_sysclk_cfg >> 4) & 0x7)) / 60;
        case SYSCTL_CLK_GNNE_AXI:
            return (60 - ((sysctl_clk->ai_gnne_aclk_cfg >> 4) & 0x7)) / 60;
        case SYSCTL_CLK_GNNE_AXI_NOC:
            return 1;
        case SYSCTL_CLK_GNNE_AXI_MCTL:
            return 1;
        ...
        /*---------------------------NOC0------------------------------------*/
        case SYSCTL_CLK_NOC_CLK_0:
            return (double)(60 - ((sysctl_clk->noc_clk_cfg >> 4) & 0x7)) / 60;
        case SYSCTL_CLK_NOC_CLK_0_DIV_2:
            return 2;
        case SYSCTL_CLK_NOC_CLK_0_DIV_3:
            return 3;
...
EXPORT_SYMBOL_GPL(sysctl_clk_get_leaf_div);
...
EXPORT_SYMBOL_GPL(sysctl_clk_set_phase);
...
EXPORT_SYMBOL_GPL(sysctl_clk_get_phase);
...
EXPORT_SYMBOL_GPL(sysctl_clk_get_leaf_freq);
...
EXPORT_SYMBOL_GPL(sysctl_clk_find_approximate);
